...
public function pdf(
    ...
    */
    private function buildLedgerData(
        ...
        /*
 * ------------------------------------------------------
 * Legacy financial year start
 * ------------------------------------------------------
 *
 * Acceder does not use a Jan-Dec accounting year.
 * Read the financial year containing the report From date
 * from the legacy database so brought-forward balances only
 * include movement from the beginning of that financial year.
 */
/*
 * ------------------------------------------------------
 * Legacy financial year start
 ...
 *
 * Acceder does not use a Jan-Dec accounting year.
 * Read the financial year from the LEGACY database.
 *
 * IMPORTANT:
 * tblfinancialyears exists on the legacy connection,
 * not necessarily on the HBA ERP/default connection.
 */
...
try {
    ...
    if (
        $legacySchema->hasTable('tblfinancialyears')
    ) {
        $financialYear =
            DB::connection('legacy')
...
/*
 * Safe fallback:
 * if the legacy financial-year table cannot be read,
 * preserve the existing behavior rather than breaking the ledger.
 */
        ...
        $lines =
            DB::table(
                ...
                ->where(function ($query) {
                    /*
                     * Native HBA invoice journal rows are always retained.
                     */
                    $query->where(
                        'l.legacy_data',
                        'like',
                    ...
                    /*
                     * For historical invoice rows, keep them only when
                     * no native HBA INV journal exists for that invoice.
                     *
                     * Untouched legacy invoices therefore remain visible.
                     */
                    ->orWhereNotExists(function ($native) {
                        $native
                            ->selectRaw('1')
                            ->from('journal_entries as nj')
                            ->whereColumn(
                                'nj.invoice_id',
                                'l.invoice_id'
                            )
 ...
 * ----------------------------------------------
 *
 * Acceder's brought-forward balance is based on the
 * CURRENT financial year, not the entire lifetime of
 * the account.
 *
 * Therefore:
 *   - transactions before FY start are ignored here
 *   - transactions from FY start through day before From
 *     are included in B/F
 * ----------------------------------------------
 */
if (
    ...
    if (
        $financialYearStart === null
        ||
        $postingDate >= $financialYearStart
    ) {
        if ($selectedCurrency !== null) {
            $foreignBalance +=
                (float) $foreign['debit']
...
/*
 * Acceder foreign-currency ledger only uses actual foreign amounts.
 * Do NOT convert a PKR/base amount into SAR/etc. merely because
 * an exchange rate exists.
 *
 * Explicit foreign_debit / foreign_credit values were already handled
 * above. If they are zero, this line has no foreign-currency movement.
 */
return null;
...
try {
    ...
    // Production does not have the legacy Acceder database.
    // The core ledger calculation is still valid, so simply
    // skip the optional legacy invoice-ticket enrichment.
    return $result;
}

...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request): InertiaResponse
    {
        ...
        $nativeFinancials = empty($nativeInvoiceIds)
            ...
            : DB::table('invoice_transactions')
                ...
                ->selectRaw('COALESCE(SUM(COALESCE(rate_vendor, vendor_rate, 0) + COALESCE(fare_2, 0) + COALESCE(fare_3, 0)), 0) AS payable')
                ->selectRaw(
                    "COALESCE(SUM(
        ...
        if (
            ...
            $lines = collect($context['tickets'])
                ->map(function ($ticket) use ($context, $clientCode, $legacyNumber): array {
                    ...
                    $masterRows = $context['master_by_subid'][(string) $transactionId] ?? collect();
    ...
    /*
     * Hotel is SAR-only for this WhatsApp feature for now.
     */
    $currencyCode =
        strtoupper(
            trim(
    ...
    if (
        ...
        return response()->json([
            'message' =>
                'Hotel WhatsApp lines must use SAR.',
        ], 422);
    }
...
public function storeFromWhatsAppVisa(
    ...
    /*
     * Visa WhatsApp creation is SAR-only for this first version.
     */
    if (
        $currencyCode !== 'SAR'
    ) {
        return response()->json([
            'message' =>
                'Visa WhatsApp invoices must use SAR.',
        ], 422);
    }
...
public function addVisaLineFromWhatsApp(
    ...
    if (
        ...
        return response()->json([
            'message' =>
                'Visa WhatsApp lines must use SAR.',
        ], 422);
    }
...
public function storeFromWhatsAppTransfer(
    ...
    /*
     * Transfer WhatsApp command is SAR-only for now.
     */
    if (
        $currencyCode !== 'SAR'
    ) {
        return response()->json([
            'message' =>
                'Transfer WhatsApp invoices must use SAR.',
        ], 422);
    }
...
public function addTransferLineFromWhatsApp(
    ...
    if (
        ...
        return response()->json([
            'message' =>
                'Transfer WhatsApp invoice lines must use SAR.',
        ], 422);
    }
...
private function resolveOrCreateWhatsAppHotel(
    ...
    private function validateInvoice(Request $request): array
    {
        $data = $request->validate([
            ...
            'lines.*.receivable_amount' => ['required', 'numeric', 'min:0.01'],
...
'lines.*.vendor_rate_per_night' => ['nullable', 'numeric', 'min:0'],
        ...
        foreach ($data['lines'] as $index => &$line) {
            ...
            if ($mode === 'Ticket') {
                ...
                $line['vendor_amount'] = round(
    (float) ($line['vendor_rate_per_night'] ?? 0) * $units,
    4
);

                if ($vendor - $baseSale > 0.00005) {
                    throw ValidationException::withMessages([
                        "lines.{$index}.vendor_amount" => 'Vendor total cannot exceed the Ticket base sale (receivable less agent commission).',
                    ]);
                }
                ...
                if ($mode === 'Hotel') {
                    ...
                    // Visa commission is included within the gross customer
                    // charge and separately posted to the agent account.
                    if ($vendor + $agentAmount - $baseSale > 0.00005) {
                        throw ValidationException::withMessages([
                            "lines.{$index}.vendor_amount" => 'Vendor plus agent commission cannot exceed the gross Visa sale amount.',
                        ]);
                    }

                    $receivable = $baseSale;
                    $profit = round($baseSale - $vendor - $agentAmount, 4);
                } else {
                    ...
                    if ($vendor - $baseSale > 0.00005) {
                        throw ValidationException::withMessages([
                            "lines.{$index}.vendor_amount" => 'Vendor total cannot exceed the Transfer customer sale amount.',
                        ]);
                    }
                ...
                if ($profit < -0.00005) {
                    throw ValidationException::withMessages([
                        "lines.{$index}.vendor_amount" => 'Vendor cost cannot exceed the service sale amount.',
                    ]);
                }
            } elseif ($mode === 'Other') {
                ...
                if ($vendor - $baseSale > 0.00005) {
                    throw ValidationException::withMessages([
                        "lines.{$index}.vendor_amount" => 'Vendor total cannot exceed the Other service base sale.',
                    ]);
                }
                ...
                if ($vendor - $receivable > 0.00005) {
                    throw ValidationException::withMessages([
                        "lines.{$index}.vendor_amount" => 'Vendor total cannot exceed the receivable amount on a sales line.',
                    ]);
                }
            ...
            if ($profit > 0.00005 && trim((string) ($line['revenue_account_code'] ?? '')) === '') {
                throw ValidationException::withMessages([
                    "lines.{$index}.revenue_account_code" => 'Select the income/service account for this line.',
                ]);
            }

            $this->assertOptionalAccountType($line['payable_account_code'] ?? null, 'vendor', $index, 'payable_account_code');
            $this->assertOptionalAccountType($line['payable_account_2'] ?? null, 'vendor', $index, 'payable_account_2');
            $this->assertOptionalAccountType($line['payable_account_3'] ?? null, 'vendor', $index, 'payable_account_3');
        ...
        if ($totalReceivable <= 0.00005) {
            throw ValidationException::withMessages([
                'lines' => 'Enter at least one sales line with a receivable amount.',
            ]);
        }
    ...
    private function legacyInvoiceProfit(
        ...
        $clientCode = $this->resolveLegacyClientCode($context, $clientCode);
        $customerRows = $this->legacyCustomerMasterRows($context['master'] ?? collect(), $clientCode);
        if ($customerRows->isNotEmpty()) {
            $profit = (float) $customerRows->sum(fn ($row) => (float) ($row->{'Profit'} ?? 0));
            if (abs($profit) > 0.00005) return $profit;
        }

        if ($invoiceNumber !== null) {
            $normalized = $this->legacyNormalizedFinancials($invoiceNumber, $clientCode);
            if ($normalized['profit'] !== null && abs((float) $normalized['profit']) > 0.00005) {
                return (float) $normalized['profit'];
            }
        }
    ...
    private function decorateTransaction(object $row): array
    {
        ...
        $agentAmount = (float) ($row->agent_amount ?? 0);
        $mode = (string) ($row->mode ?? 'Other');
        $profit = in_array(strtoupper(trim($mode)), ['HOTEL', 'VISA', 'TRANSFER', 'TICKET'], true)
            ? $receivable - $vendorTotal - $agentAmount
            : $receivable - $vendorTotal;

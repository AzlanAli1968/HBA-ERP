...
class OtherReportsController extends Controller
{
    ...
    private function buildCheckIn(string $name, string $from, string $to, string $filterType, string $filterValue): array
    {
        ...
        foreach ($sourceLines as $line) {
            ...
            $mode = strtoupper(trim((string) ($this->legacyValue($meta, 'Mode') ?? $line->mode_description ?? '')));
            if ($mode !== 'HOTEL') {
                continue;
            }

            $checkInIso = $this->legacyDateIso($this->legacyValue($meta, 'ServiceDateFrom'));
            ...
            $hotelRows[] = [
                ...
                'check_out_iso' => $this->legacyDateIso($this->legacyValue($meta, 'ServiceDateTo')),
            ];
        }

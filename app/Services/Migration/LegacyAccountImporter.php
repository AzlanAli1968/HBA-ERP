...
class LegacyAccountingImporter
{
    ...
    private function importMasterAccounting(
        ...
        /*
         * Reconcile imported Master lines with the current legacy Master table.
         *
         * The importer uses upsert() for current rows, but upsert() cannot remove
         * rows that existed in an older BAK and no longer exist in the latest one.
         * Only rows carrying legacy_master_id are imported legacy Master lines, so
         * native HBA journal lines (which use NULL here) are never touched.
         *
         * The legacy and application databases currently live on the same MariaDB
         * server, so this comparison can be done efficiently in one SQL statement
         * without loading all 35k+ source IDs into PHP memory.
         */
        $legacyDatabase =
            DB::connection('legacy')->getDatabaseName();

        if ($legacyDatabase === null || $legacyDatabase === '') {
            throw new RuntimeException(
                'Legacy database name could not be resolved for Master reconciliation.'
            );
        }

        $quotedLegacyDatabase =
            '`' .
            str_replace('`', '``', $legacyDatabase) .
            '`';

        $stats['master_lines_deleted_stale'] =
            DB::affectingStatement(
                "DELETE l FROM journal_entry_lines l " .
                "LEFT JOIN {$quotedLegacyDatabase}.`Master` m " .
                "ON m.`ID` = l.`legacy_master_id` " .
                "WHERE l.`legacy_master_id` IS NOT NULL " .
                "AND m.`ID` IS NULL"
            );
    }

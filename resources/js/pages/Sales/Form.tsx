...
function lineReceivable(line: Partial<Line>): number {
    if (line.mode === 'Hotel') {
        return hotelSellingTotal(line) + numberValue(line.agent_amount);
    }
    if (line.mode === 'Transfer') {
        return transferSellingTotal(line) + numberValue(line.agent_amount);
    }
    if (line.mode === 'Other') {
        return otherBaseSale(line) + numberValue(line.agent_amount);
    }
...
export default function Form({ formMode, invoice, lines: initialLines, master, modes, statuses }: Props) {
    ...
    const selectedProfit = useMemo(() => {
        ...
        const agent = ['Hotel', 'Transfer', 'Ticket', 'Other'].includes(selectedLine?.mode ?? '')
            ...
            current.map((line, currentIndex) => {
                ...
                if (next.mode === 'Hotel') {
                    ...
                    next.receivable_amount =
                        sellingTotal + numberValue(next.agent_amount);
                ...
                if (next.mode === 'Visa') {
                    if (key === 'rate' || key === 'quantity') {
                        next.receivable_amount =
                            ...
                            Math.max(numberValue(next.quantity), 1);
                    }

                    next.currency_quantity = Math.max(
                ...
                if (next.mode === 'Ticket') {
                    const quantity = Math.max(numberValue(next.quantity), 1);
                    const rate = Math.max(numberValue(next.rate), 0);
                    const existingBaseSale = ticketBaseSale(next);

                    // Ticket customer rate is the base sale before the
                    // optional agent commission. When rate is entered,
                    // gross receivable = base sale + agent commission.
                    if (
                        key === 'rate'
                        || key === 'quantity'
                        || key === 'agent_amount'
                        ...
                        next.receivable_amount =
                            baseSale + numberValue(next.agent_amount);
                    }
                ...
                if (next.mode === 'Other') {
                    const quantity = Math.max(numberValue(next.quantity), 1);
                    const rate = Math.max(numberValue(next.rate), 0);
                    const existingBaseSale = otherBaseSale(next);

                    // Other services use an optional base sale/rate. Agent
                    // commission is added to the customer-side gross charge.
                    if (
                        key === 'rate'
                        || key === 'quantity'
                        || key === 'agent_amount'
                ...
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)_260px]">
                        ...
                        <div className="space-y-4">
                            ...
                            <Section title="Payable / Vendor" icon={<WalletCards className="h-4 w-4" />}>
                                {selectedLine.mode === 'Hotel' ? (
                                    <div className="grid gap-4 md:grid-cols-4">

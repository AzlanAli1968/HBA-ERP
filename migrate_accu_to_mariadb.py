...
def main() -> int:
    args = parse_args()
    print("=" * 78)
    print("HBA ERP — Accu-Travel -> MariaDB Legacy Mirror")
    print("=" * 78)
    ...
    try:
        ...
        # The legacy mirror is rebuilt table-by-table. Disable FK checks
        # during replacement/load so parent tables can be dropped before
    ...
    try:
        ...
        for idx, ((schema, table), info) in enumerate(
            infos.items(),
            start=1,
        ):
            started = utc_now()
            ...
            try:
                ...
                # If the table exists and not replacing, truncate so the
                # snapshot remains deterministic rather than accumulating
                # duplicates.
                cur = maria_conn.cursor()
                ...
                if exists:
                    ...
                    try:
                        cur.execute(
                            f"TRUNCATE TABLE {q_mysql(table)}"
                        )
                        maria_conn.commit()
                    finally:
                        cur.close()
                ...
                if target_count != info.source_row_count:
                    raise RuntimeError(
                        ...
                        f"source={info.source_row_count:,}, "
        ...
        # Phase 1 is complete. Re-enable FK checks before rebuilding
        ...
        print(
            "Full table/data mirror completed successfully."
        )

        print(
            "Next step: map this legacy mirror into the HBA ERP "
            "application tables."
        )
        ...
        # Always restore MariaDB foreign-key checking, even if the
